...
- **Логи ошибок в TUI** — единый экран ошибок демона и терминального интерфейса
...
### Защита от дубликатов аккаунта MAX

При новой авторизации maxBridge сверяет фактический числовой идентификатор профиля MAX
(`payload.profile.contact.id`) с уже сохранёнными сессиями. Если этот профиль уже
зарегистрирован под другим локальным аккаунтом, новая регистрация отклоняется, а созданная
для неё сессия не сохраняется. Проверенный идентификатор `max_contact_id` хранится внутри
того же зашифрованного файла сессии, что и device ID и токен, а не открытым текстом.

Для legacy-конфигураций, где один профиль уже сохранён несколько раз, демон обрабатывает
аккаунты последовательно в порядке конфигурации: первая сохранённая регистрация становится
канонической и единственной получает право производить события. Более поздние дубликаты
отключаются, и их producer остаётся выключенным; файлы сессий автоматически не удаляются.

После обновления пакета на сервере перезапустите один выбранный supervisor демона, чтобы
защита применялась в новом процессе. Не запускайте параллельно второй демон через TUI,
PM2 или systemd.
...
4. Нажмите 🧪 Тест Bot API — он проверяет только прямой запрос к Telegram Bot API, но не готовность daemon forwarding
5. Включите переключатель 🟢
6. Уже работающий демон применит сохранённые настройки автоматически без перезапуска.
...
Демон отслеживает атомарные изменения `data/telegram.json`: сохранение или переключение
настроек в TUI включает и отключает только MAX→Telegram forwarding без ручного
перезапуска. Временно отсутствующий или повреждённый файл не заменяет последнюю корректную
runtime-конфигурацию. Telegram control bot читает этот же файл только при старте демона:
изменение его токена, `allowed_chat_ids` или polling-настроек требует перезапуска выбранного
единственного daemon/supervisor процесса. Кнопка `Тест Bot API` выполняет только прямую
проверку доступности Telegram Bot API и не подтверждает готовность daemon forwarding.
...
Путь Telegram-состояния определяется выбранным основным конфигом, а не текущим каталогом
процесса. Для checkout-конфига `cli/config/local.yaml` TUI, PM2 и прямой CLI используют
один `cli/data/telegram.json`. Для системного `/etc/maxbridge/config.yaml` unit с
`StateDirectory=maxbridge` использует `/var/lib/maxbridge/data/telegram.json`; checkout TUI
не читает и не изменяет этот отдельный системный файл.
...
TUI подключается к уже выбранному демону. При checkout-local запуске TUI фиксирует рабочим
каталогом `cli`, передаёт демону явный `-c cli/config/local.yaml` и использует те же
`data`, `logs`, session/key и application PID paths. Переключатель демона в TUI не
заменяет systemd или PM2: если процесс уже управляется supervisor-ом, не запускайте из TUI
второй демон.

Пункт `📋 Логи` объединяет две локальные области ошибок: все сохранённые записи уровня
`ERROR`/`CRITICAL` из `logs/maxbridge-errors.log` относительно runtime root и до 100
текущих структурированных ошибок демона из IPC-кольца (`rpc`, `log`, `attachment`). Демон
и TUI дописывают в канонический файл без очистки; файл создаётся с правами `0600`, а
Telegram-токены, Authorization/Bearer, API keys, `auth_secret`, `bot_token`, access,
refresh, ID и client token/secret, а также token, secret и password значения редактируются
при записи и повторно перед показом. Полные traceback-блоки сохраняются в записи.

Экран обновляется автоматически и остаётся полезным без работающего демона: история из
файла показывается вместе со статусом недоступного IPC. Эта область намеренно не включает
`WARNING`/`INFO`, `journalctl`, системные или удалённые логи, а также уже удалённую или
ротированную историю. Экран не предоставляет очистку, удаление, редактирование или экспорт.
...
не требует его удаления. Telegram-конфиг этого systemd-процесса находится в
`/var/lib/maxbridge/data/telegram.json`, внутри объявленного `StateDirectory`; checkout TUI
его не администрирует. Не запускайте systemd одновременно с локальным процессом или PM2.
...
foreground-команда выше. Сохранение, включение или отключение Telegram-настроек атомарно
перезагружает только forwarder уже работающего демона. Telegram control bot и его
единственный polling lease не перезапускаются и не перенастраиваются: изменения control-bot
конфигурации вступят в силу после перезапуска supervisor-owned процесса. PM2 продолжает
писать в свои `cli/logs/maxbridge-out.log` и
`cli/logs/maxbridge-error.log`; не запускайте TUI-демон, когда владельцем процесса уже
является PM2 или systemd.
